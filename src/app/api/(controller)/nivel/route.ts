 ...
 */
async function copiarEstructuraNivel(
  idPadreOrigen: number,
  idPadreDestino: number,
  esPrimerNivel: boolean = true
): Promise<void> {
  console.log(
    `🔄 Copiando estructura de padre origen ${idPadreOrigen} a destino ${idPadreDestino} (Primer nivel: ${esPrimerNivel})`
  );
  ...
  console.log(
    `📊 Encontrados ${nivelesHijos.length} niveles hijos a copiar:`,
    nivelesHijos.map((n) => ({ IDN: n.IDN, NOMBRE: n.NOMBRE }))
  );
  ...
  for (const hijo of nivelesHijos) {
    ...
    console.log(
      `✅ Nivel creado exitosamente: ${hijo.NOMBRE} con nuevo IDN: ${nuevoNivelId}`
    );
    ...
    console.log(
      `📋 Encontradas ${actividadesHijo.length} actividades para el nivel ${hijo.NOMBRE}`
    );

    for (const actividad of actividadesHijo) {
      console.log(
        `  📌 Copiando actividad: ${actividad.DESCRIPCION} (IDA origen: ${actividad.IDA})`
      );
      ...
      if (nuevaActividadResult.length === 0) {
        console.log(
          `    ❌ Error al insertar actividad ${actividad.DESCRIPCION}`
        );
        continue;
      }
      ...
      console.log(
        `    ✅ Actividad insertada con nuevo IDA: ${nuevaActividadId}`
      );
      ...
      if (atributos.length > 0) {
        console.log(
          `    📎 Copiando ${atributos.length} atributos-valor para la actividad ${actividad.DESCRIPCION}`
        );
    ...
    console.log(
      `🔁 Llamando recursivamente para copiar sub-niveles de ${hijo.NOMBRE} (${hijo.IDN} -> ${nuevoNivelId})`
    );
    await copiarEstructuraNivel(hijo.IDN, nuevoNivelId, false); // false = no es primer nivel, copiar todo
  }

  console.log(
    `✅ Completado copiado de estructura de padre ${idPadreOrigen} a ${idPadreDestino}`
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchParams.has("UNIDAD_MANTENIBLE")) {
      filters.UNIDAD_MANTENIBLE =
        searchParams.get("UNIDAD_MANTENIBLE") === "true";
    }
    ...
    return NextResponse.json(
      {
        ...
        data: null,
        message:
          error instanceof Error ? error.message : "Error al obtener niveles",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.PLANTILLA && body.IDNP) {
      console.log(
        `🎯 Nivel creado como PLANTILLA, iniciando copia de estructura...`
      );
      console.log(
        `   Padre (IDNP): ${body.IDNP}, Nuevo nivel (IDN): ${nuevoNivel.IDN}`
      );
      ...
      try {
        ...
        console.log(
          ...
          padre.length > 0
            ? {
                IDN: padre[0].IDN,
                NOMBRE: padre[0].NOMBRE,
                GENERICO: padre[0].GENERICO,
              }
            : "No encontrado"
        );
        ...
        if (padre.length > 0 && padre[0].GENERICO) {
          ...
          console.log(
            `❌ Padre no es genérico o no existe, no se copiará estructura`
          );
        }
      } catch (copyError) {
    ...
    return NextResponse.json(
      {
        ...
        data: null,
        message:
          error instanceof Error ? error.message : "Error al crear nivel",
      },
      { status: 500 }
    );
  }

...
const TreeNode = ({
    ...
    const handleChevronClick = (e: React.MouseEvent) => {
        e.stopPropagation()
        // Toggle expansion usando el estado controlado del Accordion
    ...
    const isExpanded = value.includes(item.id)
        ...
        >
            <AccordionPrimitive.Item value={item.id}>
                <AccordionPrimitive.Header>
                    <div
                        className={cn(
                            'flex flex-1 w-full items-center py-1 transition-all',
                            treeVariants(),
                    ...
                    >
                        ...
                        <button
                            ...
                            className="shrink-0 mr-1 p-0.5 hover:bg-accent rounded transition-colors"
                            aria-label={isExpanded ? 'Colapsar' : 'Expandir'}
                        >
                            <ChevronRight 
                                className={cn(
                                    "h-4 w-4 transition-transform duration-200 text-accent-foreground/50",
                                    isExpanded && "rotate-90"
                                )} 
                            />
                        </button>
                        
                        {/* Contenido del item - clickable para seleccionar */}
                        <div

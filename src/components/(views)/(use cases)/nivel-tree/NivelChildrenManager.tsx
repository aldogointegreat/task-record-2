...
import { Folder, Plus, Pencil, Trash2, Download, Zap, X } from "lucide-react";
import {
  createNivel,
  updateNivel,
  deleteNivel,
...
export function NivelChildrenManager({
  ...
  const [quickAddItems, setQuickAddItems] = useState<QuickAddNivelItem[]>([]);
  ...
  const handleOpenQuickAdd = () => {
    const defaultJerarquia = jerarquias[0]?.IDJ || 0;
    setQuickAddItems([
      { id: crypto.randomUUID(), nombre: "", idj: defaultJerarquia },
      { id: crypto.randomUUID(), nombre: "", idj: defaultJerarquia },
      { id: crypto.randomUUID(), nombre: "", idj: defaultJerarquia },
    ]);
  ...
  const handleQuickAddSave = async () => {
    // Filtrar items con nombre válido
    const validItems = quickAddItems.filter(
      (item) => item.nombre.trim() !== ""
    );

    if (validItems.length === 0) {
      toast.error("Agrega al menos un nivel con nombre");
      return;
    }
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`${successCount} nivel(es) creado(s) exitosamente`);
      ...
      if (errorCount > 0) {
        toast.error(`${errorCount} nivel(es) no se pudieron crear`);
      }
    } catch (error) {
      console.error("Error creating niveles:", error);
      toast.error("Error al crear niveles");
    } finally {
  ...
  const handleOpenImport = async () => {
    ...
    try {
      ...
      if (!abuelo.GENERICO) {
        toast.error(
          "El padre de esta plantilla no es un nivel Genérico, no se pueden importar componentes"
        );
        setIsImporting(false);
        return;
      }
      ...
      const siblings = siblingsResult.data.filter(
        (n) => !n.PLANTILLA && !n.GENERICO && n.IDN !== nivelPadre.IDN
      );

      const currentChildrenNames = localNiveles.map((n) =>
        n.NOMBRE.toLowerCase()
      );

      const available = siblings.filter(
        (s) => !currentChildrenNames.includes(s.NOMBRE.toLowerCase())
      );
  ...
  const handleImport = async (componente: Nivel) => {
    try {
      ...
      if (result.success) {
        toast.success(
          `Componente "${componente.NOMBRE}" importado exitosamente`
        );

        // Eliminar de la lista local de disponibles
        setAvailableComponents((prev) =>
          prev.filter((n) => n.IDN !== componente.IDN)
        );
  ...
  const handleCreate = async () => {
    try {
      ...
      if (formData.PLANTILLA) {
        toast.info(
          `Se copiará la estructura de "${nivelPadre.NOMBRE}" excluyendo plantillas y genéricos`
        );
      }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title="Agregar varios niveles rápidamente"
          >
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({ ...formData, IDJ: Number(value) })
                }
              >
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({
                    ...formData,
                    ID_DISCIPLINA_NIVEL:
                      value === "__null__" ? null : Number(value),
                  })
                }
              >
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, NROPM: Number(e.target.value) })
                }
                placeholder="0"
              />
            </div>
            ...
            <div>
              <Label htmlFor="id_usr" className="mb-2 block">
                ID Usuario
              </Label>
              <Input
                id="id_usr"
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    ID_USR: e.target.value ? Number(e.target.value) : undefined,
                  })
                }
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!formData.NOMBRE || !formData.IDJ}
            >
              Crear
            </Button>
          </DialogFooter>
      ...
      <Dialog open={isEditing} onOpenChange={setIsEditing}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 pt-2">
            <div>
              <Label htmlFor="edit-nombre" className="mb-2 block">
                Nombre *
              </Label>
              <Input
                id="edit-nombre"
            ...
            <div>
              <Label htmlFor="edit-jerarquia" className="mb-2 block">
                Jerarquía *
              </Label>
              <Select
                value={formData.IDJ.toString()}
            ...
            <div>
              <Label htmlFor="edit-disciplina-nivel" className="mb-2 block">
                Disciplina
              </Label>
              <Select
                value={
            ...
            <div>
              <Label htmlFor="edit-nropm" className="mb-2 block">
                Número de PM
              </Label>
              <Input
                id="edit-nropm"
            ...
            <div>
              <Label htmlFor="edit-comentario" className="mb-2 block">
                Comentario
              </Label>
              <Input
                id="edit-comentario"
            ...
            <div>
              <Label htmlFor="edit-icono" className="mb-2 block">
                Ícono
              </Label>
              <IconPicker
                value={formData.ICONO || undefined}
            ...
            <div>
              <Label htmlFor="edit-id_usr" className="mb-2 block">
                ID Usuario
              </Label>
              <Input
                id="edit-id_usr"
          ...
          <div className="space-y-4 pt-2">
            {isLoadingComponents ? (
              ...
              <div className="py-8 text-center text-sm text-muted-foreground">
                ...
                <br />
                Todos los componentes del genérico ya están presentes o no
                existen componentes válidos.
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {jerarquias.find((j) => j.IDJ === comp.IDJ)
                            ?.DESCRIPCION || "Sin jerarquía"}
                        </div>
                      </div>
                    </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Agregar Niveles Rápido
            </DialogTitle>
            <DialogDescription>
              Agrega múltiples niveles a la vez. Solo completa nombre y
              jerarquía.
            </DialogDescription>
          </DialogHeader>
            ...
            {quickAddItems.map((item, index) => (
              <div key={item.id} className="flex gap-2 items-center">
                <Input
                  value={item.nombre}
                  onChange={(e) =>
                    handleQuickAddItemChange(item.id, "nombre", e.target.value)
                  }
                  placeholder="Nombre"
                ...
                />
                <Select
                  value={item.idj.toString()}
                  onValueChange={(value) =>
                    handleQuickAddItemChange(item.id, "idj", Number(value))
                  }
                >
          ...
          <DialogFooter className="mt-4">
            ...
            <Button
              ...
              disabled={
                isQuickAddSaving ||
                quickAddItems.every((item) => !item.nombre.trim())
              }
            >
              {isQuickAddSaving
                ? "Guardando..."
                : `Crear ${
                    quickAddItems.filter((i) => i.nombre.trim()).length
                  } nivel(es)`}
            </Button>
          </DialogFooter>
        </DialogContent>
